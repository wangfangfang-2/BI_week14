...
import requests
def get_page_content(request_url):
    header={header={'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.131 Safari/537.36'}}
    html = requests.get(request_urt,headers=header,timeout=10)
requests_url = 'https://ditie.mapbar.com/beijing_line/'
soup = get_page_content(requests_url)
"""
...
def find_lowest_cost_node(costs):
    ...
    for node in costs :
        ...
        if not node in processed:
            ...
            if costs[node]<lowest_costs:
                lowest_costs = costs[node]
